    ...
    <v-form @submit.prevent="submit" :disabled="isSubmitting">
      ...
      <v-row>
        <v-col cols="12" class="btns-nonstart">
          <v-btn @click="start()" v-if="!isStart">開始寫譜</v-btn>
        </v-col>
      </v-row>
      <!-- 樂譜部分-------------------------------------------- -->
      <v-row id="scoreArea" class="h-100" >
        ...
        <v-col cols="12" class="btns-start">
          <v-btn id="btn-plus" @click="addSection()" :disabled="!isDisabled" v-if="isStart" prepend-icon="mdi-plus">新增小節</v-btn>
          <v-btn type="submit" :loading="isSubmitting" :disabled="!isDisabled" color="green" v-if="isStart">上傳鼓譜</v-btn>
        </v-col>
      </v-row>
    </v-form>
  </v-container>
